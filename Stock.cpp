...
void Stock::process_order(Order* new_order_const, std::vector<Trader> &traders, const CommandLineArgs &args, long long &trades_completed)
{
    // 时间旅行者处理订单
...
bool Stock::check_match(Order* new_order, std::vector<Trader> &traders, const CommandLineArgs &args, long long &trades_completed)
{
    ...
    if (new_order->is_buy)
    {
        ...
        while (new_order->quantity > 0 && !sell_orders_queue.empty() && new_order->price >= sell_orders_queue.top()->price)
        {
            ...
            // 如果卖单还有数量，加入卖单队列；否则释放卖单
            if (matched_order->quantity > 0)
            {
                sell_orders_queue.push(matched_order);
            } else {
                // delete matched_order;
            }
        }
        // 买单还有数量，但是卖单队列空了，加入买单队列；否则释放买单
